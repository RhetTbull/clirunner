...
def test_service_new():
    ...
    result = runner.invoke(
        subparser, ["new", "--name", "1K-cat", "--dims", "2048, 512"]
    )
...
def test_service_remove():
    ...
    runner = CliRunner()
    result = runner.invoke(subparser, ["remove", "--name", "1K-cat"])
    assert result.exit_code == 0
...
def test_service_no_subcommand():
    ...
    runner = CliRunner()
    result = runner.invoke(subparser, [])
...
def test_service_new_missing_name():
    ...
    runner = CliRunner()
    result = runner.invoke(subparser, ["new", "--dims", "2048, 512"])
    assert result.exit_code != 0
...
def test_service_new_missing_dims():
    ...
    runner = CliRunner()
    result = runner.invoke(subparser, ["new", "--name", "test"])
    assert result.exit_code != 0
